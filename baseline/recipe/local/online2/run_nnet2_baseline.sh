...
mkdir -p exp/nnet2_online/nnet_a_gpu_online
cp -pr $dir/conf exp/nnet2_online/nnet_a_gpu_online/conf
ln -s $dir/ivector_extractor exp/nnet2_online/nnet_a_gpu_online/ivector_extractor

if [ $stage -le 4 ]; then
  # Decode.  The --per-utt true option makes no difference to the results here.
  steps/online/nnet2/decode.sh --cmd "$decode_cmd" --nj 8 \
    --per-utt true $graph_dir data/test $dir/decode_utt || exit 1;
fi
