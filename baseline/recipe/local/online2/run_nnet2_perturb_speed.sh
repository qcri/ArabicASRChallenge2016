...
if [ $stage -le -1 ]; then
  utils/perturb_data_dir_speed.sh 0.9 data/train data/train_p
  
  steps/make_mfcc.sh --cmd "$train_cmd" --nj 20 data/train_p exp/make_mfcc/train_p mfcc_perturbed || exit 1;
    steps/compute_cmvn_stats.sh data/train_p exp/make_mfcc/train_p mfcc_perturbed || exit 1;
  done
fi

if [ $stage -le 0 ]; then
  steps/align_fmllr.sh --nj 30 --cmd "$train_cmd" \
    data/train_p data/lang exp/tri4b exp/tri4b_ali_si284p || exit 1;
fi
...
if [ $stage -le 2 ]; then
  ...
  steps/online/nnet2/train_ivector_extractor.sh --cmd "$train_cmd" --nj 10 \
    data/train_p $nnet_dir/diag_ubm $nnet_dir/extractor || exit 1;
fi

if [ $stage -le 3 ]; then
   steps/online/nnet2/extract_ivectors_online.sh --cmd "$train_cmd" --nj 30 \
    data/train_p $nnet_dir/extractor $nnet_dir/ivectors_train_p || exit 1;
fi

if [ $stage -le 4 ]; then
  steps/nnet2/train_pnorm_simple2.sh --stage $train_stage \
    --online-ivector-dir $nnet_dir/ivectors_train_p \
    --num-epochs 4 \
    ...
    date/train_p data/lang exp/tri4b_ali_si284p $dir  || exit 1;
fi
